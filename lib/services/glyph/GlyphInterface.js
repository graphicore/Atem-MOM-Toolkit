define([
  ...
  , emitterMixin
    ...
    _p._makeLayer = function(key) {
        ...
        switch(key) {
            ...
            case ('cpsui'):
                ...
                subscription = this._drawCPSUI(layerElement, this._mom);
                break;
            default:
    ...
    function* cpsuiPropertiesGenerator(styleDict) {
        var keys = styleDict.keys
          , key, i, l, value
          ;
        for(i=0,l=keys.length;i<l;i++) {
            key = keys[i];
            value = styleDict.get(key, null);
            // TODO: make a better type check here!
            if(!value || typeof value !== 'object' || value.constructor.name !== 'UI')
                continue;
            yield key;
        }
    }
    ...
    //CPS*UI//
    //////////
    _p._drawCPSUI = function(layer, momGlyph) {
        var interfaces = []
          , stack = [momGlyph]
        ...
        while( (child = stack.pop()) ) {
            ...
            // TODO: cpsuiPropertiesGenerator should have a "filter"
            // argument, so that we get only data types back that are
            // supported here.
            gen = cpsuiPropertiesGenerator(styleDict);
            while(!(generatorItem = gen.next()).done) {
                ...
                dom.classList.add('cpsui', 'type-' + child.type, 'cps-ui-type-vector'
                                                , 'cps-ui-property-' + propertyName);
                // add dom event handlers

                // We could as well be event-handler saving here and delegate
    ...
    _p._destructors = {
      ...
      , cpsui: function (items) {
            console.warn('Destroying the cpsui layer is not implemented. Items are: ', items);
        }
    };
