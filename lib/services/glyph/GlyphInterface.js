define([
  ...
  , 'Atem-MOM/cpsTools'
  , 'Atem-Property-Language/flavours/MOM/UI'
  ...
  , Vector
    ...
    _p._cpsUIInitDrag = function(interfaces, initalEvent) {
    ...
    function cpsUIPropertyChange(propertyName, item) {
        ...
        // update
        if(item.uiItem === uiItem)
            console.log('item.uiItem === uiItem, but they should differ!', item.uiItem, uiItem);
        item.uiItem = uiItem;
    ...
    function updateElementCPSUIs(data, channelKey, eventData) {
        ...
        var properties = data[1]
          // element = data[0]
          , seen = new Set()
          , i, l, propertyName, item
          ;
        for(i=0,l=eventData.length;i<l;i++) {
            propertyName = eventData[i];
            if(seen.has(propertyName)
                            // This is now only for properties that
                            // we already use. Will have to change.
                            || !properties.has(propertyName))
                continue;
            seen.add(propertyName);
            item = properties.get(propertyName);
            if(!cpsUIPropertyChange(propertyName, item))
    ...
    function drawCPSUIVector(item) {
        ...
        // make the value of the ui element relative to the first
        // argument after the value and thus make the relative positioning
        // explicit in CPS.
        // NOTE: we don't need this for implicit relative coordinates.
        // We can just subtract the relative part from the value when
        // needed.
        // FIXME: this needs a decision if we want this
        // FIXME: this needs documentation
        //
        // Maybe, this is a nice idea, but it makes stuff more complicated
        // so it doesn't obey the KISS principle (needs extra documentation)
        //
        // it's also a nice way to make the relative part "portable" when
        // the uiItem is referenced
        //
        // HOWEVER: the other way around may not be obvious for users
        // and wouldn't necessarily end up in documentation ...
        //
        //
        //
    ...
    _p._initCPSUI = function(layer, momGlyph) {
        var interfaces = []
          ...
          , subscription = {
                elements: elements
              , elementMousedown: this._cpsUIInitDrag.bind(this, interfaces)
            }
          , stack = [momGlyph]
          , child, styleDict, gen, generatorItem, propertyName, uiItem
          , item, dom, properties
          ;
        ...
        while( (child = stack.pop()) ) {
            ...
            while(!(generatorItem = gen.next()).done) {
                propertyName = generatorItem.value;
                ...
                if(!elements.has(child)) {
                    ...
                    elements.set(child, {
                      ...
                      , subscriptionID: styleDict.on('change', updateElementCPSUIs, [child, properties])
                    });

                }
                else
                    properties = elements.get(child).properties;
                ...
                dom.classList.add('cpsui', 'type-' + child.type, 'cps-ui-type-vector');


                // add dom event handlers

                // We could as well save event-handlers here and delegate
                // this to the parent element as it is done with the on click
                // handlers for surfaces and points.
                // Once a drag is initiated, the process is pretty much
