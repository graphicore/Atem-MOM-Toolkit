define([
  ...
  , 'Atem-MOM-Toolkit/dataTransformationCaches/SVGUsePathsProvider'
  , 'Atem-MOM-Toolkit/dataTransformationCaches/SVGUseCustomProvider'
  , './GlyphInterface'
  , './layers/cpsUI'
  , './layers/shapes'
  , './layers/points'
  ...
  , points
    ...
    var svgns = 'http://www.w3.org/2000/svg'
      , centerlineRenderer = {
            penstroke: renderingBasics.renderPenstrokeCenterline
        }
      , metaLinesRenderer
      ;
    ...
    function drawLine(path, from, to) {
        var d = path.getAttribute('d')
          , segment = ['M', from[0], from[1], 'L', to[0], to[1]].join(' ')
          ;
        path.setAttribute('d', (d ? d + ' ' : '') + segment);
    }
    ...
    function GlyphUIService(document, drawPointsOutlineProvider) {
        ...
        this._layers.full = new SVGUsePathProvider(this._svg, prefix+'full', this.services.outline, true);
        // draw the the centerlines (of penstrokes) as path
        this._layers.centerline = new SVGUsePathProvider(this._svg, prefix+'centerline', this.services.centerline, false);
        // draws a lot of clutter, will be refined soon
        // this information should only be rendered to selected elements ...
        this._layers.meta = new SVGUseCustomProvider(this._svg, prefix+'meta', metaLinesRenderer);

        var shapesLayer = shapes(this.services.shapes);
        this._layers.shapes = [shapesLayer.create, shapesLayer.destroy];
        this._layers.points = [points.create, points.destroy];
        this._layers.cpsui = [cpsUI.create, cpsUI.destroy];
    }

    var _p = GlyphUIService.prototype;

    _p.get = function(glyph, options) {
        var canvas = new GlyphInterface(this._doc, this._layers, glyph, options);
        return canvas;
    };
